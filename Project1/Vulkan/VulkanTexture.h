...
public:
	...
	void UploadDataToImage(void* tex_data, uint32_t size, VkCommandPool cmdPool, VkQueue graphQueue, MemoryAllocator *p_vkMemory);
	void GenerateMipChain(uint32_t mipLevels, VkCommandPool cmdPool, VkQueue graphQueue);
	void LoadTexture(std::string filename, const VkSamplerAddressMode addrMode, float maxAnisotropy, const VkBorderColor color, const VkFormat format, const VkCommandPool cmdPool, VkQueue graphQueue, MemoryAllocator *p_vkMemory);
	void LoadTextureArray(std::string filename, VkSamplerAddressMode addrMode, float maxAnisotropy, const VkBorderColor color, const VkFormat format, const VkCommandPool cmdPool, VkQueue graphQueue, MemoryAllocator *p_vkMemory);
	void LoadCubeMap(std::string filename, const VkFormat format, const VkCommandPool cmdPool, VkQueue graphQueue, MemoryAllocator *p_vkMemory);
