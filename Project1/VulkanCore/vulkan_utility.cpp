...
VulkanUtility::VulkanUtility(VulkanScene *vulkanScene) :
...
void VulkanUtility::CreateBuffer(uint32_t size, VkBufferUsageFlags flags, VkMemoryPropertyFlags props, VkBuffer& buffer, VkDeviceMemory& devMemory)
{
	...
	VK_CHECK_RESULT(vkCreateBuffer(p_vulkanScene->m_device.device, &createInfo, nullptr, &buffer));

	VkMemoryRequirements memoryReq;
	vkGetBufferMemoryRequirements(p_vulkanScene->m_device.device, buffer, &memoryReq);
	...
	VK_CHECK_RESULT(vkAllocateMemory(p_vulkanScene->m_device.device, &memoryInfo, nullptr, &devMemory));

	VK_CHECK_RESULT(vkBindBufferMemory(p_vulkanScene->m_device.device, buffer, devMemory, 0));
}
...
void VulkanUtility::CopyBuffer(VkBuffer dest, VkBuffer src, VkDeviceSize size, VkCommandPool cmdPool)
{
	...
	this->EndCmdBuffer(cmdBuffer, cmdPool, p_vulkanScene->m_queue.graphQueue);
}
...
uint32_t VulkanUtility::FindMemoryType(uint32_t type, VkMemoryPropertyFlags flags)
{
	...
	vkGetPhysicalDeviceMemoryProperties(p_vulkanScene->m_device.physDevice, &memoryProp);
...
{
	int imageCount = p_vulkanScene->m_imageView.images.size();
	...
	for (int c = 0; c < imageCount; ++c) {
		...
		if (imageView == VK_NULL_HANDLE) {
			attach.resize(1);
			attach = { p_vulkanScene->m_imageView.images[c] };
		}
		else {
			attach.resize(2);
			attach = { p_vulkanScene->m_imageView.images[c], imageView };
		}
		...
		VK_CHECK_RESULT(vkCreateFramebuffer(p_vulkanScene->m_device.device, &frameInfo, nullptr, &frameBuffers[c]));
	}
...
VkCommandPool VulkanUtility::InitCommandPool(uint32_t index)
{
	...
	VK_CHECK_RESULT(vkCreateCommandPool(p_vulkanScene->m_device.device, &commandInfo, nullptr, &cmdPool));
...
VkCommandBuffer VulkanUtility::CreateCmdBuffer(bool primary, bool singleUse, VkFramebuffer frameBuffer, VkRenderPass renderPass, VkCommandPool cmdPool)
{
	...
	VkCommandBuffer cmdBuffer;
	VK_CHECK_RESULT(vkAllocateCommandBuffers(p_vulkanScene->m_device.device, &allocInfo, &cmdBuffer));
...
void VulkanUtility::EndCmdBuffer(VkCommandBuffer cmdBuffer, VkCommandPool cmdPool, VkQueue queue)
{
	...
	vkFreeCommandBuffers(p_vulkanScene->m_device.device, cmdPool, 1, &cmdBuffer);
}
...
{
	vkFreeCommandBuffers(p_vulkanScene->m_device.device, cmdPool, cmdBuffer.size(), cmdBuffer.data());
}

// =========================================================================================================================================================================================================================================================================================================
// pipeline utilities

VkPipelineViewportStateCreateInfo VulkanUtility::InitViewPort(VkViewport& viewPort, VkRect2D& scissor, float width, float height)
{
	viewPort.x = 0.0f;
	viewPort.y = 0.0f;
...
VkShaderModule VulkanUtility::CreateShaderModule(std::vector<char>& shader)
{
	...
	VK_CHECK_RESULT(vkCreateShaderModule(p_vulkanScene->m_device.device, &shaderInfo, nullptr, &shaderModule));
...
TextureInfo VulkanUtility::LoadTexture(std::string filename, VkSamplerAddressMode addrMode, VkCompareOp compare, float maxAnisotropy, VkBorderColor color, VkFormat format, VkCommandPool cmdPool)
{
	...
	void *data;
	vkMapMemory(p_vulkanScene->m_device.device, stagingMemory, 0, texture.size, 0, &data);
	...
	VK_CHECK_RESULT(vkCreateImage(p_vulkanScene->m_device.device, &image_info, nullptr, &texture.image));

	VkMemoryRequirements mem_req;
	vkGetImageMemoryRequirements(p_vulkanScene->m_device.device, texture.image, &mem_req);
	...
	VK_CHECK_RESULT(vkAllocateMemory(p_vulkanScene->m_device.device, &alloc_info, nullptr, &texture.texture_mem));

	vkBindImageMemory(p_vulkanScene->m_device.device, texture.image, texture.texture_mem, 0);
	...
	this->EndCmdBuffer(comm_buff, cmdPool, p_vulkanScene->m_queue.graphQueue);
	...
	texture.imageView = p_vulkanScene->InitImageView(texture.image, format, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_VIEW_TYPE_2D);

	CreateTextureSampler(texture, addrMode, 16, compare, color);

	vkDestroyBuffer(p_vulkanScene->m_device.device, staging_buff, nullptr);
...
TextureInfo VulkanUtility::LoadTextureArray(std::string filename, VkSamplerAddressMode addrMode, VkCompareOp compare, float maxAnisotropy, VkBorderColor color, VkFormat format, VkCommandPool cmdPool)
{
	...
	texture.imageView = p_vulkanScene->InitImageView(texture.image, format, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_VIEW_TYPE_2D_ARRAY);
...
void VulkanUtility::CreateTextureSampler(TextureInfo& texture, VkSamplerAddressMode addressMode, float maxAnisotropy, VkCompareOp compareOp, VkBorderColor borderColor)
{
	...
	VK_CHECK_RESULT(vkCreateSampler(p_vulkanScene->m_device.device, &sampler_info, nullptr, &texture.m_tex_sampler));
}
...
VkFormat VulkanUtility::FindSupportedFormat(const std::vector<VkFormat>& requiredFormats, VkImageTiling tiling, VkFormatFeatureFlags features)
{
	for (auto format : requiredFormats) {
		...
		vkGetPhysicalDeviceFormatProperties(p_vulkanScene->m_device.physDevice, format, &props);
...
uint32_t VulkanUtility::InitRenderFrame()
{
	...
	vkAcquireNextImageKHR(p_vulkanScene->m_device.device, p_vulkanScene->m_swapchain.swapChain, std::numeric_limits<uint64_t>::max(), p_vulkanScene->m_semaphore.image, VK_NULL_HANDLE, &image_index);
...
void VulkanUtility::SubmitFrame(uint32_t imageIndex)
{
	...
	present_info.pWaitSemaphores = &p_vulkanScene->m_semaphore.render;
	present_info.swapchainCount = 1;
	present_info.pSwapchains = &p_vulkanScene->m_swapchain.swapChain;
	present_info.pImageIndices = &imageIndex;

	VK_CHECK_RESULT(vkQueuePresentKHR(p_vulkanScene->m_queue.presentQueue, &present_info));

	VK_CHECK_RESULT(vkQueueWaitIdle(p_vulkanScene->m_queue.presentQueue));
}