...
VulkanWater::VulkanWater(VulkanEngine *engine, VulkanUtility *utility) :
	VulkanModule(utility),
	p_vkEngine(engine)
{
	p_FFT = new vkFFT(p_vkEngine);
...
void VulkanWater::PrepareOffscreenFrameBuffer()
{
	...
	m_normalInfo.renderpass.PrepareRenderPass(p_vkEngine->m_device.device);

	m_normalInfo.renderpass.PrepareFramebuffer(m_normalInfo.image.imageView, DISPLACEMENT_MAP_SIZE, DISPLACEMENT_MAP_SIZE, p_vkEngine->m_device.device);
}

void VulkanWater::PrepareOffscreenBuffers()
{
	// buffers for normal pipeline
	m_buffers.offscreen.normUbo.CreateBuffer(sizeof(NormalUbo), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);

	NormalUbo normUbo;
	normUbo.choppiness = m_waterParams.choppiness;
	normUbo.gridLength = DISPLACEMENT_MAP_SIZE / m_waterParams.patchLength;
	m_buffers.offscreen.normUbo.MapBuffer<NormalUbo>(normUbo, p_vkEngine->GetDevice());
}
...
void VulkanWater::PrepareOffscreenDescriptorSets()
{
	...
	VK_CHECK_RESULT(vkCreateDescriptorPool(p_vkEngine->m_device.device, &createInfo, nullptr, &m_normalInfo.descriptors.pool));
	...
	VK_CHECK_RESULT(vkCreateDescriptorSetLayout(p_vkEngine->m_device.device, &layoutInfo, nullptr, &m_normalInfo.descriptors.layout));
	...
	VK_CHECK_RESULT(vkAllocateDescriptorSets(p_vkEngine->m_device.device, &allocInfo, &m_normalInfo.descriptors.set));

	std::array<VkDescriptorBufferInfo, 1> bufferInfo;
	bufferInfo[0] = vkUtility->InitBufferInfoDescriptor(m_buffers.offscreen.normUbo.buffer, 0, m_buffers.offscreen.normUbo.size);										// ubo
...
void VulkanWater::PrepareOffscreenPipeline()
{
	...
	VK_CHECK_RESULT(vkCreatePipelineLayout(p_vkEngine->m_device.device, &pipelineInfo, nullptr, &m_normalInfo.pipelineInfo.layout));
	...
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(p_vkEngine->m_device.device, VK_NULL_HANDLE, 1, &createInfo, nullptr, &m_normalInfo.pipelineInfo.pipeline));
}
...
void VulkanWater::PrepareBuffers()
{
	...
	m_waterInfo.uboTese.CreateBuffer(sizeof(TerrainTeseUbo), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);
	
	// fragment shader ubo
	m_waterInfo.uboFrag.CreateBuffer(sizeof(TerrainFragUbo), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);

	// map data to device for fragment shader now as it will remain constant
	TerrainFragUbo ubo;
	ubo.perlinOctave = PERLIN_OCTAVE;
	ubo.perlinMovement = PERLIN_SPEED;
	ubo.perlinGradient = PERLIN_GRADIENT;
	ubo.texelSize2x = m_waterParams.patchLength / DISPLACEMENT_MAP_SIZE * 2;
	m_waterInfo.uboFrag.MapBuffer<TerrainFragUbo>(ubo, p_vkEngine->GetDevice());
...
void VulkanWater::PrepareDescriptorSets()
{
	...
	VK_CHECK_RESULT(vkCreateDescriptorPool(p_vkEngine->m_device.device, &createInfo, nullptr, &m_waterInfo.descriptors.pool));
	...
	VK_CHECK_RESULT(vkCreateDescriptorSetLayout(p_vkEngine->m_device.device, &layoutInfo, nullptr, &m_waterInfo.descriptors.layout));
	...
	VK_CHECK_RESULT(vkAllocateDescriptorSets(p_vkEngine->m_device.device, &allocInfo, &m_waterInfo.descriptors.set));

	std::array<VkDescriptorBufferInfo, 2> bufferInfo;
	bufferInfo[0] = vkUtility->InitBufferInfoDescriptor(m_waterInfo.uboTese.buffer, 0, m_waterInfo.uboTese.size);															
	bufferInfo[1] = vkUtility->InitBufferInfoDescriptor(m_waterInfo.uboFrag.buffer, 0, m_waterInfo.uboFrag.size);														
...
void VulkanWater::PreparePipeline()
{
	...
	VK_CHECK_RESULT(vkCreatePipelineLayout(p_vkEngine->m_device.device, &pipelineInfo, nullptr, &m_waterInfo.pipelineInfo.layout));
	...
	VK_CHECK_RESULT(vkCreateGraphicsPipelines(p_vkEngine->m_device.device, VK_NULL_HANDLE, 1, &createInfo, nullptr, &m_waterInfo.pipelineInfo.pipeline));
}
...
void VulkanWater::GenerateWaterCmdBuffer(VkCommandBuffer cmdBuffer, VkDescriptorSet set, VkPipelineLayout layout, VkPipeline pipeline)
{
	...
	vkCmdBindVertexBuffers(cmdBuffer, 0, 1, &m_waterInfo.vertices.buffer, bgOffsets);
	vkCmdBindIndexBuffer(cmdBuffer, m_waterInfo.indices.buffer, 0, VK_INDEX_TYPE_UINT32);
...
void VulkanWater::PrepareComputeBuffers()
{
	// spectrum compute
	// stroage buffers
	m_specComputeInfo.ssboBuffer.dx.CreateBuffer(sizeof(glm::vec2) * TOTAL_DISPLACEMENT_SIZE, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);
	m_specComputeInfo.ssboBuffer.dy.CreateBuffer(sizeof(glm::vec2) * TOTAL_DISPLACEMENT_SIZE, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);
	m_specComputeInfo.ssboBuffer.ht.CreateBuffer(sizeof(glm::vec2) * TOTAL_DISPLACEMENT_SIZE, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);

	// ubo buffer
	m_specComputeInfo.uboBuffer.CreateBuffer(sizeof(ComputeUbo), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);

	// map ubo data to local device
	ComputeUbo ubo;
	ubo.dismapDim = DISPLACEMENT_MAP_SIZE;
	ubo.time = 0;								// zero time point at initialisation
	m_specComputeInfo.uboBuffer.MapBuffer<ComputeUbo>(ubo, p_vkEngine->GetDevice());

	// buffers for displacement compute pipeline
	m_dispComputeInfo.uboBuffer.CreateBuffer(sizeof(DisplacementUbo), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);

	DisplacementUbo disUbo;
	disUbo.choppiness = m_waterParams.choppiness;
	disUbo.mapSize = DISPLACEMENT_MAP_SIZE;
	m_dispComputeInfo.uboBuffer.MapBuffer<DisplacementUbo>(disUbo, p_vkEngine->GetDevice());
...
void VulkanWater::PrepareSpecComputeDescriptors()
{
	...
	bufferInfo[0] = vkUtility->InitBufferInfoDescriptor(m_buffers.heightMap.buffer, 0, m_buffers.heightMap.size);							// h(0)
	bufferInfo[1] = vkUtility->InitBufferInfoDescriptor(m_buffers.omegaMap.buffer, 0, m_buffers.omegaMap.size);								// omega
	bufferInfo[2] = vkUtility->InitBufferInfoDescriptor(m_specComputeInfo.ssboBuffer.ht.buffer, 0, m_specComputeInfo.ssboBuffer.ht.size);			// h(t)
	bufferInfo[3] = vkUtility->InitBufferInfoDescriptor(m_specComputeInfo.ssboBuffer.dx.buffer, 0, m_specComputeInfo.ssboBuffer.dx.size);			// dx(t)
	bufferInfo[4] = vkUtility->InitBufferInfoDescriptor(m_specComputeInfo.ssboBuffer.dy.buffer, 0, m_specComputeInfo.ssboBuffer.dy.size);			// dy(t)
	bufferInfo[5] = vkUtility->InitBufferInfoDescriptor(m_specComputeInfo.uboBuffer.buffer, 0, m_specComputeInfo.uboBuffer.size);					// ubo
...
void VulkanWater::PrepareDispComputeDescriptorSets()
{
	...
	VK_CHECK_RESULT(vkCreateDescriptorPool(p_vkEngine->m_device.device, &createInfo, nullptr, &m_dispComputeInfo.descriptors.pool));
	...
	VK_CHECK_RESULT(vkCreateDescriptorSetLayout(p_vkEngine->m_device.device, &layoutInfo, nullptr, &m_dispComputeInfo.descriptors.layout));
	...
	VK_CHECK_RESULT(vkAllocateDescriptorSets(p_vkEngine->m_device.device, &allocInfo, &m_dispComputeInfo.descriptors.set));

	std::array<VkDescriptorBufferInfo, 4> bufferInfo;
	bufferInfo[0] = vkUtility->InitBufferInfoDescriptor(m_dispComputeInfo.uboBuffer.buffer, 0, m_dispComputeInfo.uboBuffer.size);					// ubo
	bufferInfo[1] = vkUtility->InitBufferInfoDescriptor(m_specComputeInfo.ssboBuffer.dx.buffer, 0, m_specComputeInfo.ssboBuffer.dx.size);		// dx(t) - buffers derived from compute shader calcultaions
	bufferInfo[2] = vkUtility->InitBufferInfoDescriptor(m_specComputeInfo.ssboBuffer.dy.buffer, 0, m_specComputeInfo.ssboBuffer.dy.size);		// dy(t)
	bufferInfo[3] = vkUtility->InitBufferInfoDescriptor(p_FFT->m_ssboDstBuffer.buffer, 0, p_FFT->m_ssboDstBuffer.size);							// fft	
	std::array<VkDescriptorImageInfo, 1> imageInfo;
	imageInfo[0] = vkUtility->InitImageInfoDescriptor(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, m_dispComputeInfo.image.imageView, m_dispComputeInfo.image.texSampler);	// displacement map texture sampler
...
void VulkanWater::PrepareSpecComputePipeLine()
{
	...
	VK_CHECK_RESULT(vkCreatePipelineLayout(p_vkEngine->m_device.device, &pipelineInfo, nullptr, &m_specComputeInfo.pipelineInfo.layout));
	...
	VK_CHECK_RESULT(vkCreateComputePipelines(p_vkEngine->m_device.device, VK_NULL_HANDLE, 1, &createInfo, nullptr, &m_specComputeInfo.pipelineInfo.pipeline));
}
...
void VulkanWater::PrepareDispComputePipeLine()
{
	...
	VK_CHECK_RESULT(vkCreatePipelineLayout(p_vkEngine->m_device.device, &pipelineInfo, nullptr, &m_dispComputeInfo.pipelineInfo.layout));
	...
	VK_CHECK_RESULT(vkCreateComputePipelines(p_vkEngine->m_device.device, VK_NULL_HANDLE, 1, &createInfo, nullptr, &m_dispComputeInfo.pipelineInfo.pipeline));
}

void VulkanWater::GenerateSpecComputeCmdBuffers()
{
	m_specComputeInfo.cmdBuffer = vkUtility->CreateCmdBuffer(VulkanUtility::VK_PRIMARY, VulkanUtility::VK_MULTI_USE, VK_NULL_HANDLE, VK_NULL_HANDLE, p_vkEngine->m_computeCmdPool);
	...
	computeBarrier.buffer = m_buffers.heightMap.buffer;
	bufferBarriers[0] = computeBarrier;
	computeBarrier.buffer = m_buffers.omegaMap.buffer;
	bufferBarriers[1] = computeBarrier;
	computeBarrier.buffer = m_specComputeInfo.ssboBuffer.ht.buffer;
	bufferBarriers[2] = computeBarrier;
	computeBarrier.buffer = m_specComputeInfo.ssboBuffer.dx.buffer;
	bufferBarriers[3] = computeBarrier;
	computeBarrier.buffer = m_specComputeInfo.ssboBuffer.dy.buffer;
...
{
	m_dispComputeInfo.cmdBuffer = vkUtility->CreateCmdBuffer(VulkanUtility::VK_PRIMARY, VulkanUtility::VK_MULTI_USE, VK_NULL_HANDLE, VK_NULL_HANDLE, p_vkEngine->m_computeCmdPool);
...
void VulkanWater::SubmitWaterCompute()
{
	...
	VK_CHECK_RESULT(vkQueueSubmit(p_vkEngine->m_queue.computeQueue, 1, &submitInfo, m_specComputeInfo.fence));
	...
	VK_CHECK_RESULT(vkQueueSubmit(p_vkEngine->m_queue.computeQueue, 1, &submitInfo, m_dispComputeInfo.fence));
...
void VulkanWater::GenerateHeightMap()
{
	...
	m_buffers.heightMap.CreateBuffer(vecSize * sizeof(glm::vec2), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);
	m_buffers.heightMap.MapBuffer<glm::vec2>(heightMap, p_vkEngine->m_device.device);

	m_buffers.omegaMap.CreateBuffer(vecSize * sizeof(float), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);
	m_buffers.omegaMap.MapBuffer<float>(omegaMap, p_vkEngine->m_device.device);
}
...
void VulkanWater::GenerateMeshData()
{
	...
	// vertex
	m_waterInfo.vertices.CreateBuffer(vertices.size() * sizeof(TerrainUtility::Vertex), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);
	m_waterInfo.vertices.MapBuffer<TerrainUtility::Vertex>(vertices, p_vkEngine->GetDevice());

	//index
	m_waterInfo.indices.CreateBuffer(indices.size() * sizeof(uint32_t), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, p_vkEngine);
	m_waterInfo.indices.MapBuffer<uint32_t>(indices, p_vkEngine->GetDevice());
...
void VulkanWater::LoadWaterAssets()
{
	...
	m_waterInfo.noiseImage.LoadTexture("assets/textures/perlin_noise.ktx", VK_SAMPLER_ADDRESS_MODE_REPEAT, 16.0f, VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE, VK_FORMAT_BC3_UNORM_BLOCK, p_vkEngine->m_cmdPool, p_vkEngine);
}
...
void VulkanWater::Update(int acc_time)
{
	...
	ubo.cameraPos = glm::vec4(camera->GetCameraPosition(), 1.0f);
	ubo.modelMatrix = glm::mat4(1.0f);
	ubo.screenDim = glm::vec2(p_vkEngine->m_surface.extent.width, p_vkEngine->m_surface.extent.height);

	// perlin noise constants
	ubo.perlinOctave = PERLIN_OCTAVE;
	ubo.perlinAmplitude = PERLIN_AMPLITUDE;
	ubo.perlinMovement = PERLIN_SPEED;

	// constant tessellation values
	ubo.dispFactor = TESSELLATION_DISP_FACTOR;
	ubo.tessFactor = TESSELLATION_FACTOR;
	ubo.tessEdgeSize = TESSELLATION_EDGE_SIZE;
	m_waterInfo.uboTese.MapBuffer<TerrainTeseUbo>(ubo, p_vkEngine->GetDevice());

	// update the compute ubo buffer 
	// map ubo data to local device
	ComputeUbo compUbo;
	compUbo.dismapDim = DISPLACEMENT_MAP_SIZE;
	compUbo.time = acc_time * m_waterParams.time;
	m_specComputeInfo.uboBuffer.MapBuffer<ComputeUbo>(compUbo, p_vkEngine->GetDevice());
}
...
void VulkanWater::Init()
{
	...
	p_FFT->GenerateFFTCmdBuffer(m_specComputeInfo.ssboBuffer.ht.buffer);
