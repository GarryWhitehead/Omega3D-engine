...
namespace OmegaEngine
{
	...
	void AnimationManager::parseGltfAnimation(uint32_t spaceId, tinygltf::Model& model)
	{
		std::vector<AnimationInfo> animBuffer;
		...
		animationBuffer.insert(std::make_pair(spaceId, animBuffer));
	}

	void AnimationManager::parseGltfSkin(uint32_t spaceId, tinygltf::Model& model)
	{
		std::vector<SkinInfo> skinBuf;
		for (tinygltf::Skin& skin : model.skins) {
			...
			skinBuf.push_back(skinInfo);
		}
		skinBuffer.insert(std::make_pair(spaceId, skinBuf));
	}
}
