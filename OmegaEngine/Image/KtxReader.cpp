...
namespace ImageUtility
{
	...
	bool KtxReader::parse(const size_t fileSize)
	{
		...
		imageOutput.totalSize = (header.pixelWidth * header.pixelHeight * 4 * header.numberOfMipmapLevels * header.numberOffaceCount) *  header.numberOfArrayElements;
	...
	std::vector<uint8_t> KtxReader::generate(std::vector<uint8_t>& data, uint32_t width, uint32_t height, uint32_t arrayCount, uint32_t faceCount, uint32_t mipLevels)
	{
	...
	bool KtxReader::saveFile(const char* filename, std::vector<uint8_t>& data, uint32_t mipLevels, uint32_t arrayCount, uint32_t num_faceCount, uint32_t width, uint32_t height)
	{
		if (!filename) 
		{
