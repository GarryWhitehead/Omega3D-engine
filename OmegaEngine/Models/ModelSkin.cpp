...
void ModelSkin::extractSkinData(tinygltf::Model &gltfModel, tinygltf::Skin &skin,
                                std::unique_ptr<GltfModel::Model> &model, uint32_t skinIndex)
{
	...
	if (skin.inverseBindMatrices > -1)
	{
		...
		memcpy(invBindMatrices.data(), &buffer.data[accessor.byteOffset + bufferView.byteOffset],
