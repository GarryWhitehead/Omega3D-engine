...
public:
	...
	void map(vk::Device dev, vk::DeviceMemory memory, const uint32_t offset, void *data,
	         uint32_t totalSize, uint32_t mappedOffset); // for data types of *void

	template <typename T>
	void map(vk::Device dev, vk::DeviceMemory memory, const uint32_t offset,
...
public:
	...
	static constexpr float ALLOC_BLOCK_SIZE_LOCAL =
	    2.56e+8f; // each default device local block is 256mb
	...
	// helper functions
	void createBuffer(uint32_t size, vk::BufferUsageFlags flags, vk::MemoryPropertyFlags props,
	...
	void mapDataToSegment(MemorySegment &segment, void *data, uint32_t totalSize,
	                      uint32_t offset = 0);
