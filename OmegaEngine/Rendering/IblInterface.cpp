...
IblInterface::IblInterface(VulkanAPI::Interface& vkInterface)
{
	...
	specularMapTexture.createEmptyImage(vk::Format::eR32G32B32A32Sfloat, specularMapDim, specularMapDim, mipLevels,
	                                    vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eSampled |
	                                        vk::ImageUsageFlagBits::eTransferDst,
	                                    6);
}
...
{
	OEMaths::vec3f target[6] = { OEMaths::vec3f(1.0f, 0.0f, 0.0f), OEMaths::vec3f(-1.0f, 0.0f, 0.0f),
		                         OEMaths::vec3f(0.0f, 1.0f, 0.0f), OEMaths::vec3f(0.0f, -1.0f, 0.0f),
		                         OEMaths::vec3f(0.0f, 0.0f, 1.0f), OEMaths::vec3f(0.0f, 0.0f, -1.0f) };

	OEMaths::vec3f cameraUp[6] = { OEMaths::vec3f(0.0f, 1.0f, 0.0f),  OEMaths::vec3f(0.0f, 1.0f, 0.0f),
		                           OEMaths::vec3f(0.0f, 0.0f, -1.0f), OEMaths::vec3f(0.0f, 0.0f, 1.0f),
		                           OEMaths::vec3f(0.0f, 1.0f, 0.0f),  OEMaths::vec3f(0.0f, 1.0f, 0.0f) };
...
void IblInterface::generateBrdf(VulkanAPI::Interface& vkInterface)
{
	...
	if (!shader.add(vkInterface.getDevice(), "quad-vert.spv", VulkanAPI::StageType::Vertex, "Env/BRDF/lutBRDF-frag.spv",
...
void IblInterface::createSpecularMap(VulkanAPI::Interface& vkInterface)
{
	...
	// prepare the shader
	if (!state.shader.add(vkInterface.getDevice(), "env/PreFilter/specularmap-vert.spv", VulkanAPI::StageType::Vertex,
	                      "env/PreFilter/specularmap-frag.spv", VulkanAPI::StageType::Fragment))
	{
	...
	for (auto& layout : state.imageLayout.layouts)
	{
		...
		{
			state.descriptorSet.writeSet(state.imageLayout.find(0, layout.binding).value(),
	...
	vk::Viewport viewPort =
	    vk::Viewport{ 0.0f, 0.0f, static_cast<float>(specularMapDim), static_cast<float>(specularMapDim), 0.0f, 1.0f };
	cmdBuffer.setViewport(viewPort);
	cmdBuffer.setScissor();

	// transition cube texture for transfer
	specularMapTexture.getImage().transition(vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal,
	                                         cmdBuffer.get());

	offscreenTexture.getImage().transition(vk::ImageLayout::eUndefined, vk::ImageLayout::eColorAttachmentOptimal,
	                                       cmdBuffer.get());
	...
	for (uint32_t mip = 0; mip < mipLevels; mip++)
	{
		...
		for (uint32_t face = 0; face < 6; face++)
		{
			...
			vk::ImageCopy imageCopy(src_resource, { 0, 0, 0 }, dst_resource, { 0, 0, 0 },
			...
			offscreenTexture.getImage().transition(vk::ImageLayout::eColorAttachmentOptimal,
			                                       vk::ImageLayout::eTransferSrcOptimal, cmdBuffer.get());
			cmdBuffer.get().copyImage(offscreenTexture.getImage().get(), vk::ImageLayout::eTransferSrcOptimal,
			                          specularMapTexture.getImage().get(), vk::ImageLayout::eTransferDstOptimal, 1,
			                          &imageCopy);
			...
			offscreenTexture.getImage().transition(vk::ImageLayout::eTransferSrcOptimal,
			                                       vk::ImageLayout::eColorAttachmentOptimal, cmdBuffer.get());
		}
	}
	specularMapTexture.getImage().transition(vk::ImageLayout::eTransferDstOptimal,
	                                         vk::ImageLayout::eShaderReadOnlyOptimal, cmdBuffer.get());
	cmdBuffer.end();
...
void IblInterface::createIrradianceMap(VulkanAPI::Interface& vkInterface)
{
	...
	vk::Viewport viewPort = vk::Viewport{
		0.0f, 0.0f, static_cast<float>(irradianceMapDim), static_cast<float>(irradianceMapDim), 0.0f, 1.0f
	};
	cmdBuffer.setViewport(viewPort);
	cmdBuffer.setScissor();

	// transition cube texture for transfer
	irradianceMapTexture.getImage().transition(vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal,
	                                           cmdBuffer.get());
	...
	for (uint8_t mip = 0; mip < mipLevels; mip++)
	{
		for (uint8_t face = 0; face < 6; face++)
		{
			...
			cmdBuffer.bindPushBlock(state.pipelineLayout, vk::ShaderStageFlagBits::eVertex, sizeof(OEMaths::mat4f),
			                        &mvp);
			...
			offscreenTexture.getImage().transition(vk::ImageLayout::eColorAttachmentOptimal,
			                                       vk::ImageLayout::eTransferSrcOptimal, cmdBuffer.get());
			cmdBuffer.get().copyImage(offscreenTexture.getImage().get(), vk::ImageLayout::eTransferSrcOptimal,
			                          irradianceMapTexture.getImage().get(), vk::ImageLayout::eTransferDstOptimal, 1,
			                          &imageCopy);
