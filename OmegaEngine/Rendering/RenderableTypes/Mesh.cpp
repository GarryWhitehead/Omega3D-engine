    ...
    : RenderableBase(RenderTypes::StaticMesh)
{
	...
	sortKey = RenderQueue::createSortKey(RenderStage::First, primitive.materialId,
	                                     RenderTypes::StaticMesh);
	...
	meshInstance->state = stateManager->createState(vkInterface, renderer, StateType::Mesh,
	                                                mesh.topology, mesh.type, stateAlpha);
	...
	meshInstance->descriptorSet.init(vkInterface->getDevice(), *layoutInfo.layout,
	                                 layoutInfo.setValue);
	vkInterface->gettextureManager()->updateGroupedDescriptorSet(
	    meshInstance->descriptorSet, mat.name.c_str(), layoutInfo.setValue);
	...
	meshInstance->materialPushBlock.diffuseFactor =
	    OEMaths::vec3f{ mat.factors.diffuse.getX(), mat.factors.diffuse.getY(),
		                mat.factors.diffuse.getZ() };
	...
	meshInstance->materialPushBlock.haveNormalMap =
	    mat.hasTexture[(int)ModelMaterial::TextureId::Normal] ? 1 : 0;
	meshInstance->materialPushBlock.haveAoMap =
	    mat.hasTexture[(int)ModelMaterial::TextureId::Occlusion] ? 1 : 0;
	meshInstance->materialPushBlock.haveEmissiveMap =
	    mat.hasTexture[(int)ModelMaterial::TextureId::Emissive] ? 1 : 0;
...
void RenderableMesh::createMeshPipeline(std::unique_ptr<VulkanAPI::Interface> &vkInterface,
                                        std::unique_ptr<RendererBase> &renderer, MeshType type,
                                        ...
                                        StateId::StateFlags &flags)
{
	...
	{
		if (!state->shader.add(vkInterface->getDevice(), "model/model-vert.spv",
	...
	for (auto &layout : state->bufferLayout.layouts)
	{
		...
		if (layout.name == "CameraUbo")
		{
			vkInterface->getBufferManager()->enqueueDescrUpdate(
			    "Camera", &state->descriptorSet, layout.set, layout.binding, layout.type);
		}
		...
		{
			vkInterface->getBufferManager()->enqueueDescrUpdate(
			    "SkinnedTransform", &state->descriptorSet, layout.set, layout.binding, layout.type);
		}
	}
	...
	if (type == MeshType::Static)
	{
		vkInterface->gettextureManager()->bindTexturesToDescriptorLayout(
		    "Mesh", &state->descriptorLayout, materialSet);
	}
	else if (type == MeshType::Skinned)
	{
		vkInterface->gettextureManager()->bindTexturesToDescriptorLayout(
...
void RenderableMesh::render(VulkanAPI::SecondaryCommandBuffer &cmdBuffer, void *instance)
{
	...
	cmdBuffer.bindDynamicDescriptors(state->pipelineLayout, meshSet,
	                                 VulkanAPI::PipelineType::Graphics, dynamicOffsets);
	cmdBuffer.bindPushBlock(state->pipelineLayout, vk::ShaderStageFlagBits::eFragment,
	                        sizeof(MeshInstance::MaterialPushBlock),
	                        &instanceData->materialPushBlock);
