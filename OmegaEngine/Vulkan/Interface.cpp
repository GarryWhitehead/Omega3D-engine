...
namespace VulkanAPI
{
	...
	Interface::Interface(VulkanAPI::Device& dev, const uint32_t windowWidth, const uint32_t winHeight, NewFrameMode mode)
	{
		...
		swapchainKhr.create(device, gpu, dev.getSurface(), dev.getQueueIndex(Device::QueueType::Graphics),
		...
		cmdBufferManager = std::make_unique<CommandBufferManager>(device, gpu, graphicsQueue, presentionQueue, swapchainKhr, mode);
	}
