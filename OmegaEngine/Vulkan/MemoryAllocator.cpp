...
namespace VulkanAPI
{
	...
	uint32_t MemoryAllocator::allocateBlock(MemoryType type, uint32_t size)
	{
		...
		if (type == MemoryType::VK_BLOCK_TYPE_HOST) 
		{
			...
			createBuffer(allocatedSize, vk::BufferUsageFlagBits::eIndexBuffer | vk::BufferUsageFlagBits::eVertexBuffer | vk::BufferUsageFlagBits::eUniformBuffer
		...
		else if (type == MemoryType::VK_BLOCK_TYPE_LOCAL) 
		{
			...
			createBuffer(allocatedSize, vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eIndexBuffer | vk::BufferUsageFlagBits::eVertexBuffer | vk::BufferUsageFlagBits::eUniformBuffer
	...
	uint32_t MemoryAllocator::findFreeSegment(const uint32_t id, const uint32_t segmentSize)
	{
		...
		if (memoryBlocks[id].freeSegments.empty()) 
		{
			...
			memoryBlocks[id].freeSegments.insert(std::make_pair(offset + segmentSize, memoryBlocks[id].totalSize - segmentSize));
		}
		else 
		{
	...
	void MemoryAllocator::mapDataToSegment(MemorySegment &segment, void *data, uint32_t totalSize, uint32_t offset)
	{
		...
		else if (block.type == MemoryType::VK_BLOCK_TYPE_LOCAL) 
		{
			...
			createBuffer(segment.getSize(), vk::BufferUsageFlagBits::eTransferSrc, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent, tempMemory, tempBuffer);

			// map data to temporary buffer
			segment.map(device, tempMemory, 0, data, totalSize, offset);		// mem offseting not allowed for device local buffer
	...
	void MemorySegment::map(vk::Device dev, vk::DeviceMemory memory, const uint32_t offset, void* sourceData, uint32_t totalSize, uint32_t mappedOffset)
	{
