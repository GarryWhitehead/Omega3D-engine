...
namespace VulkanAPI
{
	...
	void Shader::bufferReflection(DescriptorLayout& descriptorLayout, std::vector<ShaderBufferLayout>& bufferLayout)
	{
		...
		for (uint8_t i = 0; i < (uint8_t)StageType::Count; ++i) {
			...
			for (auto& image : shaderResources.storage_images) 
			{
				...
				descriptorLayout.addLayout(set, binding, vk::DescriptorType::eStorageImage, getStageFlags(StageType(i)));
			}
		}
	}
	...
	void Shader::pipelineLayoutReflect(PipelineLayout& layout)
	{
		...
		for (uint8_t i = 0; i < (uint8_t)StageType::Count; ++i) 
		{
			...
			{
				layout.add_push_constant((StageType)i, (uint32_t)compiler.get_declared_struct_size(compiler.get_type(shaderResources.push_constant_buffers.front().base_type_id)));
			}
		}
	}
	...
	std::tuple<vk::Format, uint32_t> Shader::getTypeFormat(uint32_t width, uint32_t vecSize, spirv_cross::SPIRType::BaseType type)
	{
