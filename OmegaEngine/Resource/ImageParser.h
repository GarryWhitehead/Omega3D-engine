...
public:
	...
	uint8_t fileIdentifier[12] = { 0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31,
	...
	bool saveFile(const char *filename, std::vector<uint8_t> &data, uint32_t mipLevels,
	              uint32_t arrayCount, uint32_t num_faceCount, uint32_t width, uint32_t height);
...
private:
	...
	std::vector<uint8_t> generate(std::vector<uint8_t> &data, uint32_t width, uint32_t height,
