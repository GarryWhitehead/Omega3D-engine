...
void SceneParser::loadModels()
{
	...
	{
		LOGGER_INFO("No model data found in scene file. Assuming using object component system to "
		            "fill world?");
		return;
	}
...
void SceneParser::loadLights()
{
	...
	for (uint32_t i = 0; i < lightArray.Size(); ++i)
	{
		...
		lights[i].position =
		    OEMaths::vec3f(pos[0].GetFloat(), pos[1].GetFloat(), pos[2].GetFloat());

		auto &target = arr["Target"];
		lights[i].target =
		    OEMaths::vec3f(target[0].GetFloat(), target[1].GetFloat(), target[2].GetFloat());
