...
std::unique_ptr<OmegaEngine::ModelMesh> generateSphereMesh(const uint32_t density)
{
    ...
    mesh->primitives.push_back({0, static_cast<uint32_t>(mesh->indices.size()), -1});
    // using primitive rerstart here so when the 0xFFFF value is read, the most recent indices
...
generateCapsuleMesh(const uint32_t density, const float height, const float radius)
{
    auto mesh = std::make_unique<OmegaEngine::ModelMesh>();
    ...
    for (uint32_t i = 0; i < innerSize; ++i)
    {
        ...
        for (uint32_t j = 0; j < density; ++j)
        {
            ...
            mesh->vertices[offset + j].position = OEMaths::vec4f(
                w * radius * std::cos(rad),
                halfHeight + extraHeight,
                -w * radius * std::sin(rad),
                1.0f);

            OEMaths::vec3f norm {mesh->vertices[offset + j].position.getX(),
                                 extraHeight,
...
std::unique_ptr<OmegaEngine::ModelMesh> generateCubeMesh(const OEMaths::vec3f& size)
{
    ...
    static const std::array<OEMaths::vec3f, 36> vertexData = {
        v1, v2, v3, v3, v0, v1, v2, v6, v7, v7, v3, v2, v6, v5, v4, v4, v7, v6,
        v5, v1, v0, v0, v4, v5, v0, v3, v7, v7, v4, v0, v5, v6, v2, v2, v1, v5};

    static const std::array<OEMaths::vec2f, 36> uvData = {
        uv1, uv2, uv3, uv3, uv0, uv1, uv2, uv6, uv7, uv7, uv3, uv2, uv6, uv5, uv4, uv4, uv7, uv6,
        uv5, uv1, uv0, uv0, uv4, uv5, uv0, uv3, uv7, uv7, uv4, uv0, uv5, uv6, uv2, uv2, uv1, uv5};

    static const std::array<OEMaths::vec3f, 6> normalData = {OEMaths::vec3f {0.0f, 0.0f, +1.0f},
